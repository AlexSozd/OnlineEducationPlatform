...
AS
  BEGIN
    SELECT COUNT(*)+1 FROM ExamResults WHERE id_bl=@id_bl AND id_us=@id_us AND dt>=CAST('09/01/'+(YEAR(SYSDATETIME()) - 1) AS DATETIME);
  END;
...
AS
  BEGIN
	...
	procnt=(SELECT AVG(procnt) FROM (dbo.ExamResults JOIN dbo.Users ON ExamResults.id_us=Users.id) WHERE id_bl=(SELECT id FROM dbo.TestBlocks WHERE name=@tname));
  END;
...
AS
  BEGIN
	...
	procnt=(SELECT AVG(procnt) FROM (dbo.ExamResults JOIN dbo.TestBlocks ON ExamResults.id_bl=TestBlocks.id) WHERE id_us=(SELECT id FROM dbo.Users WHERE name=@usname));
  END;
...
AS
  BEGIN
	...
	procnt=(SELECT AVG(procnt) FROM ((dbo.ExamResults JOIN dbo.Users ON ExamResults.id_us=Users.id) JOIN dbo.TestBlocks ON ExamResults.id_bl=TestBlocks.id));
  END;
