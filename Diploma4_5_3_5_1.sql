...
(
  ...
  @name AS NVARCHAR,
  @test_av AS NVARCHAR,
  @test_res AS NVARCHAR,
  @stat_av AS NVARCHAR
)
AS
  BEGIN
    INSERT INTO dbo.UserAccess(id, name, test_av, test_results, stat_av) VALUES (@id_num, @name, @test_av, @test_res, @stat_av);
  END;
...
AS
  BEGIN
    SELECT Users.id, Users.name, UserAccess.test_av, UserAccess.test_results, UserAccess.stat_av FROM (dbo.Users JOIN dbo.UserAccess WHERE Users.name=UserAccess.name);
  END;

GO
CREATE FUNCTION GetUserAccessID
(
...
AS
  BEGIN
    DECLARE @i AS INT
    SET @i = (SELECT MAX(id)+1 FROM UserAccess);
	RETURN @i;
  END;